...
node_range domainDecomp1D( int n, int proc_dim, int proc_coord)
{
    int nlocal;
    int deficit;
    node_range limits;
                    ...
                    int & LZ)
{
    ...
    x_range = domainDecomp1D( nodes_x, dims[0], coords[0]);
    y_range = domainDecomp1D( nodes_y, dims[1], coords[1]);
    z_range = domainDecomp1D( nodes_z, dims[2], coords[2]);
